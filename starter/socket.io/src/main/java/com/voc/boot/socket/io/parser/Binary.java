...
//        packet.data = _deconstructPacket(packet.data, buffers);
        packet.setData(_deconstructPacket(packet.getData(), buffers));
        ...
        if (data instanceof byte[]) {
            ...
            try {
                ...
                if (log.isErrorEnabled()) {
                    log.error("An error occurred while putting data to JSONObject", e);
                }
                return null;
            }
            ...
            for (int i = 0; i < len; i++) {
                try {
                    ...
                    if (log.isErrorEnabled()) {
                        log.error("An error occurred while putting packet data to JSONObject", e);
                    }
                    return null;
                }
            }
...
//        packet.data = _reconstructPacket(packet.data, buffers);
        packet.setData(_reconstructPacket(packet.getData(), buffers));
